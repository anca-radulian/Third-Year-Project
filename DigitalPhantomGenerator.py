...
class DigitalPhantomGenerator:
    ...
    def __generate_pgm_files_p2(self, file_name, axis, k):
    ...
    def __determine_pgm_values(self, selected_axis, X_all, Y_all, Z_all):
        num_of_files = 0
        plane_index = 0
        selected_axis_coord = []
        plane_coord_1 = []
    ...
    def create_digital_phantom_of_models(self, selected_axis, X_all, Y_all, Z_all):
        X_all = np.array(X_all, dtype=int)
        Y_all = np.array(Y_all, dtype=int)
        Z_all = np.array(Z_all, dtype=int)
        matrix_of_files, num_of_files = self.__determine_pgm_values(selected_axis, X_all, Y_all, Z_all)
        for k in range(num_of_files):
            self.__generate_pgm_files_p2('pgm_axis_' + selected_axis, matrix_of_files, k)
